...
export function UserMessageBubble({ message, referenceUrls = [], onAddReference }: UserMessageBubbleProps) {
  return (
        ...
        {message.imageUrls && message.imageUrls.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-2">
            {message.imageUrls.map((url, i) => {
              const isRef = referenceUrls.includes(url);
              const canAdd = !isRef && referenceUrls.length < 3 && !!onAddReference;
              const atLimit = !isRef && referenceUrls.length >= 3;
              return (
                <div key={i} className="relative group h-16 w-16">
                  ...
                  {!message.isUploadingImages && (
                    isRef ? (
                      ...
                      <button
                        onClick={() => canAdd && onAddReference(url)}
                        disabled={atLimit}
